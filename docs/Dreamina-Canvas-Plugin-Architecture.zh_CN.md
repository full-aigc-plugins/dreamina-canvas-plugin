...
> | 字段 | 值 |
> |---|---|
> | 状态 | 核心契约、视觉单轮闭环与库面接线以 `0.3.0` 发布（2026-09-22 付费单轮 Canary 与 8/8 跨宿主黄金样本通过）；多轮待验 |
...
| 凭据处理 | 不拥有 | 不拥有 | 有意缺失 |
| 视觉目标与质量闭环 | 单轮已实现（付费 Canary 通过 2026-09-22）；多轮与修订自动应用未实现 | 目标锁定、单轮控制器、Judge 请求/回执、有界预算与停滞策略 | change `add-canvas-visual-quality-loop`（第 1-10 节） |
...
| 方面 | 立场 |
|---|---|
| 分发 | 指向本仓库、固定到不可变 `v0.3.0` 的跨宿主 marketplace 条目 |
